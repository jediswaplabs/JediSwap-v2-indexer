...
async def fetch_strk_grant_data_v1():
...
async def fetch_strk_grant_data_v2():
    r = redis.Redis.from_url(REDIS_URL)
    cached_data = r.get('Jediswap_V2')
    ...
    if cached_data:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(STRK_GRANT_URL) as response:
                if response.status == 200:
                    data = await response.json(content_type=None)
                    jediswap_data = data.get('Jediswap_v2', {})
                    r.set('Jediswap_V2', json.dumps(jediswap_data), ex=3600)  # Cache for 1 hour
                else:
                    return {"error": "Failed to fetch data"}
    for key, value in jediswap_data.items():
        if len(value) > 0:
